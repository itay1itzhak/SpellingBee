...
class CommonConfig(FairseqDataclass):
    ...
    path_to_load_embeddings: Optional[str] = field(
        default=None,
        metadata={
            "help": "SpellingBee: Path to load pretrained embeddings."
        },
    )
...
class CheckpointConfig(FairseqDataclass):
    ...
    bpe_pre_train: bool = field(
        default=False, metadata={"help": "Whatever to load pre-trained embeddings for bpe tokens"}
    )

    bpe_pre_train_load_file: str = field(
        default="No default for bpe_pre_train_load_file", metadata={"help": "path to load pre-trained embeddings for bpe tokens"}
    )
