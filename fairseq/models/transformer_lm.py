...
logger = logging.getLogger("fairseq.SpellingBee")
...
class TransformerLanguageModelConfig(FairseqDataclass):
    ...
    # bpeplus
    path_to_load_embeddings: Optional[str] = field(
        default=None,
        metadata={
            "help": "SpellingBee: path to load embeddings from"
        },
    )

    model_to_load_embeddings: Optional[str] = field(
        default=None,
        metadata={
            "help": "SpellingBee: Model name to load pretrained embeddings."
        },
    )
    reset_chars_emb: Optional[bool] = field(
        default=False,
        metadata={
            "help": "bpeplus: if set True, does not load embeddings for token with length 1"
        },
    )

    add_token_unicode_characters_embeddings: Optional[bool] = field(
        default=False,
        metadata={
            "help": "bpeplus: if set True, adds token positional characters embeddings"
        },
    )
...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        # bpeplus
        if args.add_token_unicode_characters_embeddings:  # bpeplus
            ...
            rand_emb_size = 8
            logger.info("rand_emb_size:" + str(rand_emb_size))
            embed_chars = torch.nn.Embedding(len(get_chars_in_vocab(task.source_dictionary)) + 1, rand_emb_size,
                                             padding_idx=0)
            # embed_chars = nn.Embedding(len(src_dict), args.encoder_embed_dim, padding_idx=src_dict.pad())
            torch.nn.init.normal_(embed_chars.weight, mean=0, std=args.decoder_embed_dim ** -0.5)
        else:
            embed_chars = None

        decoder = TransformerDecoder(
            args, task.target_dictionary, embed_tokens, no_encoder_attn=True, embed_chars=embed_chars
        )
        return cls(decoder)

    @classmethod
    def build_embedding(cls, args, dictionary, embed_dim, path=None):
        def load_pretrained_embedding(path_to_load_embeddings, embeddings_model_to_load, embed_tokens, dictionary,
                                      embed_dim):
            # name_to_load = path_to_load_embeddings  # '/home/olab/itayitzhak/bpeplus/fairseq/checkpoints/de-en_baseline/checkpoint_best.pt'
            loaded_emb_matrix = None
            loaded_from_path = torch.load(path_to_load_embeddings)
            if embeddings_model_to_load == 'roberta_large':
                assert embed_dim == 1024
                # roberta = torch.load('/home/olab/itayitzhak/bpeplus/roberta.large/model.pt')
                loaded_emb_matrix = loaded_from_path['model']['decoder.sentence_encoder.embed_tokens.weight'][:-4, :]

            elif embeddings_model_to_load == 'roberta_base':
                assert embed_dim == 768
                # roberta = torch.load('/home/olab/itayitzhak/bpeplus/roberta.base/model.pt')
                loaded_emb_matrix = loaded_from_path['model']['decoder.sentence_encoder.embed_tokens.weight'][:-4, :]

            elif embeddings_model_to_load == 'gpt2':
                # gpt2_emb = torch.load('/home/olab/itayitzhak/bpeplus/gpt2/gpt2-medium-emb.pt')
                loaded_emb_matrix = torch.cat([embed_tokens.weight.data[0:4, :], loaded_from_path], dim=0)  # [:-4, :]

            elif embeddings_model_to_load == 'arabert':
                # arabert_emb = torch.load('/home/olab/itayitzhak/bpeplus/AraBERT/bert-large-arabertv02_emb.pt')
                loaded_emb_matrix = torch.cat([
                    embed_tokens.weight.data[0, :].unsqueeze(0), loaded_from_path[0, :].unsqueeze(0),
                    embed_tokens.weight.data[2:4, :],
                    loaded_from_path[5:, :]
                ], dim=0)  # <s>, original [PAD], </s>, <ukn>, rest of embeddings

            elif embeddings_model_to_load == 'glove':
                # glove_emb = torch.load('/home/olab/itayitzhak/bpeplus/GloVe/GloVe.6B.300d.50K.emb.pt')
                loaded_emb_matrix = torch.cat(
                    [embed_tokens.weight.data[:4, :], loaded_from_path, embed_tokens.weight.data[-196:, :]], dim=0)

            elif embeddings_model_to_load == 'character_bert':
                # character_bert_emb = torch.load('/home/olab/itayitzhak/bpeplus/character-bert/emb_matrix.pt')
                loaded_emb_matrix = torch.cat([embed_tokens.weight.data[:4, :], loaded_from_path], dim=0)

            if embeddings_model_to_load == 'spellingbee_pretrained':
                logger.info("Loading embeddings pretrained by the SpellingBee probe")
                lm_model = torch.load(path_to_load_embeddings)
                loaded_emb_matrix = lm_model['model']['decoder.embed_tokens.weight']

            if loaded_emb_matrix is not None and len(dictionary) == loaded_emb_matrix.shape[0]:
                if args.reset_chars_emb:
                    logger.info("reset_chars_emb is True, not loading embeddings for tokens with length 1")
                    non_chars_tokens = [False if len(token) == 1 else True for token in dictionary.symbols]
                    embed_tokens.weight.data[non_chars_tokens] = loaded_emb_matrix[non_chars_tokens]
                else:
                    embed_tokens.weight.data = loaded_emb_matrix
                logger.info("Loaded Embeddings successfully.")
                logger.info("Freezing Embeddings")
                for p in embed_tokens.parameters():
                    p.requires_grad = False
            else:
                if loaded_emb_matrix is not None:
                    logger.info(
                        "Not loading embeddings due to different dictionary size, current dictionary size:" + str(
                            len(dictionary)))
                    logger.info("Loaded embeddings size:" + str(loaded_emb_matrix.shape[0]))
                else:
                    logger.info("Embeddings matrix was not loaded.")
                raise Exception(
                    "Pretrained embeddings failed to not loaded correctly. Check if --model-to-load-embeddings has one of the correct model names: roberta_base,roberta_large,gpt2,arabert,glove,character_bert,spellingbee_pretrained and match the file given in --path-to-load-embeddings.")

            return embed_tokens

        embed_tokens = Embedding(len(dictionary), embed_dim, dictionary.pad())
        if args.path_to_load_embeddings:
            logger.info(f"Loading {args.model_to_load_embeddings} pretrained Embeddings...")
            # logger.info(f"SpellingBee: Loading model {args.model_to_load_embeddings} from {path_to_load_embeddings}")
            embed_tokens = load_pretrained_embedding(args.path_to_load_embeddings, args.model_to_load_embeddings,
                                                     embed_tokens, dictionary, embed_dim)
...
def base_lm_architecture(args):
    ...
    # bpeplus
    # args.path_to_load_embeddings = getattr(
    #     args, "path_to_load_embeddings", None
    # )
